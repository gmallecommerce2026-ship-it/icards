...
const WeddingInvitationEditor = () => {
    ...
    const handleUserImageFileUpload = useCallback(async (filesOrData, replace = false) => {
        ...
        try {
            ...
            // Thêm ảnh mới vào đầu danh sách state sidebar
            setUserUploadedImages(prev => [...newImages, ...prev]);

            // --- CẬP NHẬT MỚI: Tự động thêm vào Canvas ---
            if (currentPageId && newImages.length > 0) {
                newImages.forEach((img) => {
                    // Gọi hàm thêm ảnh vào canvas hiện tại
                    addImageToCanvas(img.url, currentPageId);
                });
                // Chuyển công cụ về mặc định để user có thể thao tác ngay với ảnh
                setActiveTool('default'); 
            }
            // ----------------------------------------------
